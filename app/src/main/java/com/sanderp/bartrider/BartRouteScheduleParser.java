...
public class BartRouteScheduleParser {
    ...
    private static final String TAG = BartRouteScheduleParser.class.getSimpleName();
    ...
    public List<RouteSchedule> parse(InputStream in) throws XmlPullParserException, IOException {
        try {
    ...
    public List<RouteSchedule> readAPI(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        while (parser.next() !=  XmlPullParser.END_DOCUMENT) {
            ...
            if (parser.getEventType() != XmlPullParser.START_TAG) continue;
            if (name.equals(REQUEST)) departures.add(readTrip(parser));
        }
        return departures;
    }

    public RouteSchedule readTrip(XmlPullParser parser) throws XmlPullParserException, IOException {
        String origin = "";
        String origin_time = "";
        String destination = "";
        ...
        while (parser.next() !=  XmlPullParser.END_DOCUMENT) {
            if (parser.getEventType() == XmlPullParser.END_TAG && parser.getName().equals(TRIP)) break;
            if (parser.getEventType() != XmlPullParser.START_TAG) continue;
